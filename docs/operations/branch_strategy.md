...
- リリースサイクルごとに `main` から `release/X.Y.Z` を 1 本切り、変更は `release/X.Y.Z` から `feature/*` を派生させて行う。完成した feature から順に `release/X.Y.Z` へマージコミットで取り込み、最終的に `release/X.Y.Z` を `main` へ戻してタグを打ち、npm に公開する。
...
- ゲートは 2 段。CI は `.github/workflows/ci.yml` の `required-check` で、型検査、`bun test`（ユニット、Chromium を使う結合、CLI 起動）、ビルド、ビルド成果物のスモークテストを含み、`release/*` と `main` への PR の必須チェックにする。CD (npm 公開) は `npm publish` の `prepublishOnly` で型検査とテストを再実行し、1 件でも失敗すれば公開を中断する。テストが通らない状態では公開できない。

## 2. ブランチ一覧

| ブランチ | 役割 | 分岐元 | マージ先 | 公開先 | 保護 | マージ方式 |
| --- | --- | --- | --- | --- | --- | --- |
| `main` | 公開済みの現在地。タグ `vX.Y.Z` を打つ唯一のブランチ | (初期) | なし | npm (`npm publish` を手動実行。`prepublishOnly` の型検査とテストが全通過しないと公開されない) | 必須チェック、直接 push 禁止、force push 禁止、削除禁止 | マージコミット |
| `release/X.Y.Z` | リリースサイクルの統合ブランチ。feature の PR 先。同時に 1 本のみ | `main` | `main` | なし | 必須チェック、直接 push 禁止、Require up to date、force push 禁止 | マージコミット |
| `feature/<issue>_<desc>` | 個別の変更 | `release/*` | `release/*` | なし | なし | マージコミット |
...
gitGraph
    ...
    merge feature/1 id: "merge PR"
    branch feature/2
    commit id: "feat B"
    checkout release/0.1.0
    merge feature/2 id: "merge PR 2"
...
- `release/0.1.0` から `feature/*` を派生させ、完成順にマージコミットで戻す。
...
gitGraph
    ...
    commit id: "feat A"
    checkout release/0.2.0
    merge feature/A id: "merge PR"
    checkout feature/B
    commit id: "feat B-1"
    merge release/0.2.0 id: "release 取り込み"
    commit id: "feat B-2"
    checkout release/0.2.0
    merge feature/B id: "merge PR 2"
```

- feature/A と feature/B が同時に `release/0.2.0` から派生する。
- feature/A が先に完成し、`release/0.2.0` にマージコミットで取り込まれる。
- feature/B は最新の `release/0.2.0` を取り込み、コンフリクトを解消してから開発を続ける。
- feature/B が完成したら `release/0.2.0` にマージコミットで取り込む。
- マージコミットなら feature/A のコミットがそのまま `release/0.2.0` の履歴に残るため、feature/B が `release/0.2.0` を取り込んでも同じ変更を二重に解決することはない。squash だと feature/A の変更が別ハッシュの 1 コミットになり、feature/B 側で同じ差分が再びコンフリクトする。
...
| 公開時の自動テスト (CD) | `npm publish` | `package.json` の `prepublishOnly` が `bun run typecheck && bun test && bun run build` を実行する。型エラーまたはテスト失敗が 1 件でもあれば `npm publish` は中断され、パッケージは公開されない。`--ignore-scripts` で回避してはならない |

CD の前提条件として、公開対象のコミットは `main` のマージコミットであり、そのコミットに対する `required-check` (push トリガー) が成功していること。CI が失敗している状態、または未マージのブランチからは公開しない。
...
| マージ方式 | すべて merge commit。`feature/*` は squash にすると派生ブランチや release 取り込み済みブランチで同じ差分が再コンフリクトするため使わない。`release/*` と `hotfix/*` は履歴とタグの対応を残すため merge commit。squash merge と rebase merge はリポジトリ設定で無効化する |
...
- `main`: PR 必須、`required-check` 必須 (Require up to date)、force push 禁止、削除禁止、許可するマージ方式は merge のみ
- `release/**`: PR 必須、`required-check` 必須 (Require up to date)、force push 禁止、許可するマージ方式は merge のみ
...
3. `release/X.Y.Z` から `feature/*` を派生させ、`release/X.Y.Z` に PR でマージコミットする。
...
8. `main` のマージコミットに対する `required-check` (push トリガー) が成功していることを確認する (`gh run list --branch main --limit 1`)。失敗していれば公開せず、修正を hotfix として入れる。
9. `main` で `git tag vX.Y.Z && git push origin vX.Y.Z` を実行する。
10. `gospelo-github-identity check` と `npm whoami` で公開アカウントを確認し、`main` で `npm publish` を実行する。`prepublishOnly` が型検査、テスト、ビルドを順に実行し、いずれかが失敗すると公開は中断される。失敗した場合はタグを削除せず、修正を hotfix として入れてパッチバージョンで公開し直す。
11. `release/X.Y.Z` を削除する。

## 7. hotfix 手順

1. `main` から `hotfix/<issue>_<desc>` を作成する。
2. 修正と `package.json` のパッチバージョン更新、CHANGELOG の追記を入れ、`hotfix/*` -> `main` の PR を作成する。
3. `required-check` 通過後、merge commit でマージし、`main` のマージコミットに対する `required-check` の成功を確認してから `vX.Y.Z+1` タグを打って npm に公開する。`npm publish` 時も `prepublishOnly` の型検査とテストが全通過しないと公開されない。
...
| リポジトリ設定 (squash merge と rebase merge 無効、マージ後ブランチ削除) | 未 | 所有者アカウント (gospelo-dev) で実行 |
...
gh api repos/gospelo-dev/markterm --method PATCH \
  -F allow_rebase_merge=false -F allow_squash_merge=false -F allow_merge_commit=true \
